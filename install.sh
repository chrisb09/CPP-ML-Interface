...
LIBTORCH_VERSION="${SMARTSIM_LIBTORCH_VERSION:-2.4.0}"
...
if [ -d "${SMARTREDIS_DIR}" ]; then
    echo "SmartRedis directory already exists (${SMARTREDIS_DIR}). Skipping clone."

    # Check the existing repository state without relying on shell cwd.
    current_branch=$(git -C "${SMARTREDIS_DIR}" rev-parse --abbrev-ref HEAD)
    if [ "$current_branch" != "$smartsim_branch" ]; then
        ...
        git -C "${SMARTREDIS_DIR}" checkout $smartsim_branch || { echo "Failed to switch to branch $smartsim_branch. Please check the SmartRedis directory."; exit 1; }
        compile=true
    else
...
#    fi
    ...
    git clone "${smartsim_repo}" "${SMARTREDIS_DIR}" && \
    cd "${SMARTREDIS_DIR}" && \
...
installed_lib="${SMARTREDIS_DIR}/install/lib64/libsmartredis.so"
if [ ! -f "$installed_lib" ]; then
    echo "SmartRedis installed library is missing. A rebuild is required."
    compile=true
elif find "${SMARTREDIS_DIR}/include" "${SMARTREDIS_DIR}/src" "${SMARTREDIS_DIR}/CMakeLists.txt" -newer "$installed_lib" -print -quit | grep -q .; then
...
export smartredis_DIR=${SMARTREDIS_DIR}/cmake
