...
#include "include/provider/ml_coupling_provider.hpp" // MLCouplingProvider 
#include "include/normalization/ml_coupling_normalization.hpp" // MLCouplingNormalization 
#include "include/data_processor/ml_coupling_data_processor.hpp" // MLCouplingDataProcessor 
#include "include/behavior/ml_coupling_behavior.hpp" // MLCouplingBehavior 
#include "include/application/ml_coupling_application.hpp" // MLCouplingApplication 

// Includes for subclasses of MLCouplingProvider
#include "include/provider/ml_coupling_provider_aixelerate.hpp" // MLCouplingProviderAixelerate 
#include "include/provider/ml_coupling_provider_phydll.hpp" // MLCouplingProviderPhydll 
#include "include/provider/ml_coupling_provider_smartsim.hpp" // MLCouplingProviderSmartsim 

// Includes for subclasses of MLCouplingNormalization
#include "include/normalization/ml_coupling_minmax_normalization.hpp" // MLCouplingMinMaxNormalization 

// Includes for subclasses of MLCouplingDataProcessor
#include "include/data_processor/ml_coupling_data_processor_simple.hpp" // MLCouplingDataProcessorSimple 

// Includes for subclasses of MLCouplingBehavior
#include "include/behavior/ml_coupling_behavior_default.hpp" // MLCouplingBehaviorDefault 
#include "include/behavior/ml_coupling_behavior_periodic.hpp" // MLCouplingBehaviorPeriodic 

// Includes for subclasses of MLCouplingApplication
#include "include/application/ml_coupling_application_turbulence_closure.hpp" // MLCouplingApplicationTurbulenceClosure 
...
MLCouplingProvider<In, Out>* create_instance_mlcouplingprovider(const std::string &class_name, std::vector<void*> parameter) {
        ...
        if (class_name == "MLCouplingProviderAixelerate") {
            ...
            // Parameters: int host, int port, int nodes, int tasks_per_node, int cpus_per_task, int gpus_per_task
            if (parameter.size() == 6) {
                try {
                    return new MLCouplingProviderSmartsim<In, Out>(*reinterpret_cast<int*>(parameter[0]), *reinterpret_cast<int*>(parameter[1]), *reinterpret_cast<int*>(parameter[2]), *reinterpret_cast<int*>(parameter[3]), *reinterpret_cast<int*>(parameter[4]), *reinterpret_cast<int*>(parameter[5]));
                } catch (...) {
                    // Handle constructor exceptions if necessary
                }
            }
