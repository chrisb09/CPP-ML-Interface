...
inline std::vector<std::string> get_constructor_signatures(const std::string& class_name) {
    ...
    if (class_name == "MLCouplingProviderAixelerator") {
        signatures.push_back("MLCouplingProviderAixelerator(std::string model_file, int batchsize = 1, MPI_Comm app_comm = MPI_COMM_WORLD, bool enable_hybrid = false, std::optional<float> host_fraction = std::nullopt, MLCouplingData<In>* input_after_preprocessing = nullptr, MLCouplingData<Out>* output_before_postprocessing = nullptr)");
        return signatures;
    }

    if (class_name == "MLCouplingProviderSmartsim") {
        signatures.push_back("MLCouplingProviderSmartsim(std::string device, std::string model_backend, std::string model_path, std::string model_name = \"model\", std::string host = \"localhost\", int port = 6379, int nodes = 1, int num_gpus = 0, int first_gpu = 0, int batch_size = 0, int min_batch_size = 0, int min_batch_timeout = 0, const std::vector<std::string>& tf_input_labels = { }, const std::vector<std::string>& tf_output_labels = { }, MLCouplingData<In>* input_after_preprocessing = nullptr, MLCouplingData<Out>* output_before_postprocessing = nullptr)");
    ...
    if (class_name == "MLCouplingProviderPhydll") {
        signatures.push_back("MLCouplingProviderPhydll(std::string model_file, std::string backend = \"TORCH\", std::string device = \"GPU\", MLCouplingData<In>* input_after_preprocessing = nullptr, MLCouplingData<Out>* output_before_postprocessing = nullptr)");
        return signatures;
    }
...
MLCouplingProvider<In, Out>* create_instance_mlcouplingprovider(const std::string &class_name, const std::unordered_map<std::string, std::pair<int, void*>>& parameter) {
    ...
    if (resolved_class_name == "MLCouplingProviderAixelerator") {
        ...
        // Constructor with 16 parameter(s)
        ...
        // Parameters: std::string model_file, std::string backend = "TORCH", std::string device = "GPU", MLCouplingData<In>* input_after_preprocessing = nullptr, MLCouplingData<Out>* output_before_postprocessing = nullptr
        if (parameter.size() >= 1 && parameter.size() <= 5) {
            try {
                std::cout << "Creating instance of MLCouplingProviderPhydll with parameters: " << "model_file=" << config_param_cast<std::string>(parameter.at("model_file")) << ", ""backend=" << (parameter.find("backend") != parameter.end() ? config_param_cast<std::string>(parameter.at("backend")) : (std::string)"TORCH") << ", ""device=" << (parameter.find("device") != parameter.end() ? config_param_cast<std::string>(parameter.at("device")) : (std::string)"GPU") << ", ""input_after_preprocessing=" << (*reinterpret_cast<MLCouplingData<In>*>(parameter.at("input_after_preprocessing").second)) << ", ""output_before_postprocessing=" << (*reinterpret_cast<MLCouplingData<Out>*>(parameter.at("output_before_postprocessing").second)) << std::endl;
                return new MLCouplingProviderPhydll<In, Out>(config_param_cast<std::string>(parameter.at("model_file")), parameter.find("backend") != parameter.end() ? config_param_cast<std::string>(parameter.at("backend")) : (std::string)"TORCH", parameter.find("device") != parameter.end() ? config_param_cast<std::string>(parameter.at("device")) : (std::string)"GPU", *reinterpret_cast<MLCouplingData<In>*>(parameter.at("input_after_preprocessing").second), *reinterpret_cast<MLCouplingData<Out>*>(parameter.at("output_before_postprocessing").second));
            } catch (...) {
                // Handle exceptions if necessary
            }
        }
...
MLCouplingBehavior* create_instance_mlcouplingbehavior(const std::string &class_name, const std::unordered_map<std::string, std::pair<int, void*>>& parameter) {
    ...
    if (resolved_class_name == "MLCouplingBehaviorPeriodic") {
        // Constructor with 5 parameter(s)
        ...
        return nullptr;
    } else if (resolved_class_name == "MLCouplingBehaviorDefault") {
